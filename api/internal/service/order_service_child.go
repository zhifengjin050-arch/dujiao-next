...
func (s *OrderService) cancelOrderWithChildren(order *models.Order, rollbackCoupon bool) error {
	...
	err := s.orderRepo.Transaction(func(tx *gorm.DB) error {
		...
		if len(order.Children) > 0 {
			for _, child := range order.Children {
				released, err := secretRepo.ReleaseByOrder(child.ID)
				if err != nil {
					return err
				}
				if released > 0 {
					logger.Infow("stock_released",
						"order_id", child.ID,
			...
			released, err := secretRepo.ReleaseByOrder(order.ID)
			if err != nil {
				return err
			}
			if released > 0 {
		...
		if s.walletService != nil {
			if _, err := s.walletService.ReleaseOrderBalance(tx, order, constants.WalletTxnTypeOrderRefund, "????????"); err != nil {
				return err
			}
		}
		return nil
...
func (s *OrderService) CancelExpiredOrder(orderID uint) (*models.Order, error) {
	...
	logger.Infow("stock_expiry_cancel_start",
		"order_id", order.ID,
		"expires_at", order.ExpiresAt,
	)
	if err := s.cancelOrderWithChildren(order, true); err != nil {
		logger.Warnw("stock_expiry_cancel_failed",
			"order_id", order.ID,
			"error", err,
		)
		return nil, err
